...
public class utils {
    ...
    public static void takeMove(Move m, Field[][] f) {
        boolean pawn = f[m.x_from][m.y_from].who == 'b' || f[m.x_from][m.y_from].who == 'w';
        if (pawn) {
            ...
            while (fx != m.x_to && fy != m.y_to) {
                fx += (int) Math.signum(m.x_to - m.x_from);
                fy += (int) Math.signum(m.y_to - m.y_from);
    ...
    public static List<Move> filterOnlyTakes(List<Move> moves) {//do wielokrotnego wymuszonego
        if(moves==null) return null;
        List<Move> filtered = new ArrayList<Move>();
        for (Move move : moves) {
            if (move.take) {
